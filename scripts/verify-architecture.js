...
const changedFiles = execSync('git diff HEAD --name-only 2>/dev/null || git diff --name-only').toString();
  
  const liveTestTriggers = [
    'src/features/live_listener/',
  ...
  const requiresLiveTest = liveTestTriggers.some(trigger => changedFiles.includes(trigger));
